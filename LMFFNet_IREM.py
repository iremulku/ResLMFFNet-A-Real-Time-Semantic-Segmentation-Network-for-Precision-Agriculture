...
class conv5x5_resume(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride, padding, dilation=(1, 1), groups=1, bn_acti=False, bias=False):
        super().__init__()
        self.conv5x5 = Conv(nIn // 2, nIn // 2, kSize, 1, padding=2, bn_acti=True)
        self.conv1x1_resume = Conv(nIn // 2, nIn, 1, 1, padding=0, bn_acti=False)

    def forward(self, input):
        output = self.conv5x5(input)
...
class SEM_B(nn.Module):
    def __init__(self, nIn, d=1, kSize=5, dkSize=5):
        super().__init__()

        self.conv5x5 = Conv(nIn, nIn // 2, kSize, 1, padding=2, bn_acti=True)

        self.dconv_left = Conv(nIn // 4, nIn // 4, (dkSize, dkSize), 1,
                               padding=(2, 2), groups=nIn // 4, bn_acti=True)

        self.dconv_right = Conv(nIn // 4, nIn // 4, (dkSize, dkSize), 1,
                                padding=(2 * d, 2 * d), groups=nIn // 4, dilation=(d, d), bn_acti=True)


        self.bn_relu_1 = BNPReLU(nIn)

        self.conv5x5_resume = conv5x5_resume(nIn , nIn , (dkSize, dkSize), 1,
                                padding=(2 , 2 ),  bn_acti=True)
    ...
    def forward(self, input):
        ...
        output = self.conv5x5_resume(output)
...
class LMFFNet2(nn.Module):
    def __init__(self, classes=1, block_1=3, block_2=8):
        super().__init__()
    ...
    def forward(self, input):
        ...
        out_sem_block1 += out_downsample_1  # Residual connection
        ...
        out_se_block2 += out_downsample_2  # Residual connection
